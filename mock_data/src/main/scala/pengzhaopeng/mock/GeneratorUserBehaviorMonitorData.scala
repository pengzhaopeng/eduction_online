...
object GeneratorUserBehaviorMonitorData {
  ...
  def generatorMonitorData(date: String): Unit = {
    ...
    val initTimestamp: Long = JodaTimeUtil.getStrTimeMillis(date)

    // 生成看视频但是没有看完的数据
    //    writeMonitorData2File("watchVideo", false, 10000, 10000, initPhone, initTimestamp)

    // 生成看视频且看完的数据
    writeMonitorData2File("completeVideo", isCompleteVideo = true, 20001, 8000, initPhone, initTimestamp)

    // 生成看完视频且开始做作业的数据
    writeMonitorData2File("startHomework", isCompleteVideo = true, 30001, 7000, initPhone, initTimestamp)

    // 生成看完视频且做完作业的数据
    writeMonitorData2File("completeHomework", isCompleteVideo = true, 40001, 6000, initPhone, initTimestamp)

    // 生成进入订单页的数据
    writeMonitorData2File("enterOrderPage", isCompleteVideo = true, 50001, 4000, initPhone, initTimestamp)

    // 生成进入订单页且完成订单的数据
    writeMonitorData2File("completeOrder", isCompleteVideo = true, 60000, 2000, initPhone, initTimestamp)
  }
...
//    val writer = new PrintWriter(s"./${dataType}_$initTimestamp.txt")
    val writer = new PrintWriter(s"/opt/logs/${dataType}_$initTimestamp.txt")
