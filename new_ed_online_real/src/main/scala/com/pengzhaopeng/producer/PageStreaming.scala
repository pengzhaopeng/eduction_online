...
object PageStreaming {
  ...
  def main(args: Array[String]): Unit = {
    ...
    ssc.sparkContext.addFile("hdfs://nameservice1/user/....../ip2region.db")
    ssc.sparkContext.addFile(this.getClass.getResource("/ip2region.db").getPath) //广播文件,会广播到各个 executor端口的，一般可以将文件添加到HDFS中，放resource中放yran跑会找不到
